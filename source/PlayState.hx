...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		gf.x += gf.Char.positions[0];
		gf.y += gf.Char.positions[1];
		...
		{
			if (FileSystem.exists('./mods/${Assets.getText(Paths.txt('modList'))}/data/${SONG.song}'))
			{
				for (i in FileSystem.readDirectory('./mods/${Assets.getText(Paths.txt('modList'))}/data/${SONG.song}'))
				{
			...
			if (FileSystem.exists('./mods/${Assets.getText(Paths.txt('modList'))}/scripts'))
			{
				for (i in FileSystem.readDirectory('./mods/${Assets.getText(Paths.txt('modList'))}/scripts'))
				{
					if (i.contains(allowed))
					{
						var scriptrel:Array<String> = i.split('.');
		...
		timeBarBG = new FlxSprite(0, height * 0.03).loadGraphic(Paths.image('healthBar'));
		...
		timeBarTxt = new FlxText(5, height * 0.03, width, '', 18).setFormat(Paths.font("funkin.ttf"), 18, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		if (storyWeek == 6) timeBarTxt.font = 'assets/fonts/pixel.otf';
		...
		healthBar.createFilledBar(PreferencesMenu.getPref('heatlthbar-colors') ? dad.hpColor : 0xFFFF0000, PreferencesMenu.getPref('heatlthbar-colors') ? boyfriend.hpColor : 0xFF66FF33);
		add(healthBar);
		...
		if (PreferencesMenu.getPref('botplay'))
			{
				botplayTxt = new FlxText(0, scoreTxt.y - 100, 0, "BOTPLAY", 32);
		...
		versionTxt = new FlxText(5, FlxG.height - 30, 0,
			'$curSong - ${storyDifficultyText} - Week: $storyWeek - Dream Engine: ${Application.current.meta.get('version')}', 12);
		...
		if (PreferencesMenu.getPref('middlescroll')) {// im going to strangle brandon if he says nukes for kids one more time
		...
		new FlxTimer().start(3, function(tmr:FlxTimer)
			{
				...
				new FlxTimer().start(3.5, function(swaggy:FlxTimer)
				{
					camFollow.x -= 800;
					camFollow.y -= 100;
				...
				new FlxTimer().start(10, function(swagasdga:FlxTimer)
				{
	...
	function stressIntro()
	{
		...
		camHUD.visible = false;
			...
			new FlxTimer().start(31.5, function(cunt:FlxTimer) {
				FlxTween.tween(FlxG.camera, { zoom: defaultCamZoom * 1.5 }, 0.5, { ease: FlxEase.elasticOut });
				boyfriend.playAnim('singUPmiss');
				boyfriend.animation.finishCallback = function(animFinish:String) {
					FlxTween.tween(FlxG.camera, { zoom: defaultCamZoom }, 0.5, { ease: FlxEase.elasticOut });
	...
	function initDiscord():Void {
		...
		DiscordClient.changePresence(detailsText, 'Playing ' + SONG.song.toLowerCase() + " on" + " (" + storyDifficultyText + ") mode with " + songMisses + " misses", iconRPC);
		#end
	}
	...
	function schoolIntro(?dialogueBox:DialogueBox):Void
	{
		...
		var red = new FlxSprite(-100, -100).makeGraphic(FlxG.width * 2, FlxG.height * 2, dad.hpColor);
		...
		#if sys
		for (i in scripts)
			i.callFunction('schoolIntroPost');
		#end
	}
	...
	function startCountdown():Void
	{
		...
		#if sys
		for (i in scripts)
			i.callFunction('startCountdownPost');
		#end
	}
	...
	function startSong():Void
	{
		...
		DiscordClient.changePresence(detailsText, 'Playing ' + SONG.song.toLowerCase() + " on" + " (" + storyDifficultyText + ") mode with " + songMisses + " misses", iconRPC, true, songLength);
		#end
	}
	...
	public function generateSong():Void
	{
		...
		for (section in noteData) {
			for (songNotes in section.sectionNotes) {
				...
				var gottaHitNote:Bool = section.mustHitSection && songNotes[1] <= 3 || !section.mustHitSection && songNotes[1] > 3;
	...
	override function closeSubState()
	{
		if (paused)
		{
			...
			#if discord_rpc
			if (startTimer.finished)
				DiscordClient.changePresence(detailsText, 'Playing ' + SONG.song.toLowerCase() + " on" + " (" + storyDifficultyText + ") mode with " + songMisses + " misses", iconRPC, true,
	...
	{
		if (health > 0 && !paused)
			DiscordClient.changePresence(detailsPausedText, 'Playing ' + SONG.song.toLowerCase() + " on" + " (" + storyDifficultyText + ") mode with " + songMisses + " misses", iconRPC);
	...
	override public function update(elapsed:Float)
	{
			...
			if (generatedMusic)
				{
					...
					timeBarTxt.text = SONG.song + ' ( ' + FlxStringUtil.formatTime(((FlxG.sound.music.length) / 1000) - timeBarTime) + ' / ' + FlxStringUtil.formatTime(((FlxG.sound.music.length) / 1000)) + ' ) ';
				}
		...
		if (!inCutscene && !_exiting)
		{
			...
			if (health <= 0 && !practiceMode)
			{
				...
				#if discord_rpc
				DiscordClient.changePresence("Game Over - " + detailsText, SONG.song.toLowerCase() + " on" + " (" + storyDifficultyText + ") mode with" + songMisses + " misses", iconRPC);
				#end
			}
		}
	...
	public function popUpScore(strumtime:Float, daNote:Note):Void
	{
		#if sys
		for (i in scripts)
			i.callFunction('popUpScore', [daNote]);
		#end
		...
		if (noteDiff > Conductor.safeZoneOffset * Conductor.shitZone && !PreferencesMenu.getPref('botplay')){
			daRating = 'shit';
				totalNotesHit += 1 - Conductor.shitZone;
			score = 50;
		}
		else if (noteDiff > Conductor.safeZoneOffset * Conductor.badZone && !PreferencesMenu.getPref('botplay')){
			daRating = 'bad';
			score = 100;
				totalNotesHit += 1 - Conductor.badZone;
		}
		else if (noteDiff > Conductor.safeZoneOffset * Conductor.goodZone && !PreferencesMenu.getPref('botplay')){
			daRating = 'good';
	...
	public function keyShit():Void
	{
		...
		if (holdArray.contains(true) && generatedMusic || PreferencesMenu.getPref('botplay'))
		{
			notes.forEachAlive(function(daNote:Note)
			{
...
function goodNoteHit(note:Note):Void {
    #if sys
    for (i in scripts) i.callFunction('GoodNoteHit', [note]);
    #end
		...
		switch (curStage)
		{
			...
			case 'limo':
				...
				case "philly":
					...
					if (curBeat % 4 == 0) light.color = lightColors[FlxG.random.int(0, lightColors.length - 1)];
